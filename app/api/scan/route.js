...
async function scanJobs(company, taskDefs) {
  ...
  const apifyUrl = `https://api.apify.com/v2/acts/${apiActorId}/run-sync-get-dataset-items?token=${token}`;

  // Limit results to avoid 502 timeouts on companies with thousands of listings
  const input = { urls: [searchUrl], maxItems: 50 };

  let jobs = [];
  let lastError = "";

  // Retry once on 502/503 (Apify gateway timeout)
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      const ctrl = new AbortController();
      const timeout = setTimeout(() => ctrl.abort(), 50000);

      console.log(`  [JOBS] Apify call attempt ${attempt}...`);
      ...
      if (res.status === 502 || res.status === 503) {
        ...
        console.error(`  [JOBS] Apify ${lastError} — ${attempt < 2 ? "retrying..." : "giving up"}`);
      ...
      if (e.name === "AbortError") {
        lastError = "timed out (50s)";
        console.error(`  [JOBS] Apify ${lastError} — ${attempt < 2 ? "retrying..." : "giving up"}`);
        if (attempt < 2) { await new Promise(r => setTimeout(r, 2000)); continue; }
      } else {
        console.error(`  [JOBS] Apify error: ${e.message}`);
      }
      return [];
    }
  }
  ...
  // When using keyword fallback, apply company name + LinkedIn URL filter
  ...
  const companySlug = (company.linkedinSlug || "").toLowerCase();

  const signals = jobs.slice(0, 30).map(job => {
    const jobCoName = (job.companyName || "").toLowerCase().trim();
    // Also check the LinkedIn company URL in the job result
    ...
    const isMatch = hasCompanyId ? true : (
      ...
      (companySlug && jobSlug === companySlug) ||
      (companySlug && jobCoName.replace(/[^a-z0-9]/g, "").includes(companySlug.replace(/[^a-z0-9]/g, "")))
    );

    if (!isMatch && !hasCompanyId) {
      console.log(`  [JOBS] Rejected: "${job.companyName}" (slug: ${jobSlug}) ≠ "${company.name}" (slug: ${companySlug})`);
    }
