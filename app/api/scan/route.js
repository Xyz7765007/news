...
// Input: { urls: ["https://www.linkedin.com/jobs/search/?..."] }
...
async function scanJobs(company, taskDefs) {
  ...
  if (!token) {
    console.log("  [JOBS] No APIFY_TOKEN set — no real data — skipping");
    return [];
  }
  ...
  if (searchUrls.length === 0) {
    searchUrls.push(`https://www.linkedin.com/jobs/search/?keywords=${encodeURIComponent("marketing " + company.name)}&sortBy=DD`);
  }
  ...
  searchUrls.forEach((s, i) => console.log(`    [${i}] ${s}`));
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.log("  [JOBS] no real data — skipping");
      return [];
    }
    ...
    if (jobs.length === 0) {
      console.log("  [JOBS] Apify returned 0, no real data — skipping");
      return [];
    }

    // Map curious_coder output to our signal format
    // IMPORTANT: Filter to only keep jobs from the TARGET company
    // LinkedIn search for "CMO Reddit" returns jobs from ANY company, not just Reddit
    ...
    const signals = jobs.slice(0, 30).map(job => {
      ...
      return {
        ...
        _companyMatch: jobCoName.includes(companyLower) || companyLower.includes(jobCoName) || (domainBase && jobCoName.includes(domainBase)),
      };
    }).filter(j => j.jobTitle.length > 2 && j._companyMatch);

    console.log(`  [JOBS] ${jobs.length} raw → ${signals.length} matching "${company.name}"`);

    if (signals.length === 0) {
      console.log(`  [JOBS] No jobs matched company name "${company.name}" — skipping`);
      return [];
    }
    ...
    if (recentSignals.length === 0) {
      console.log("  [JOBS] No recent job posts, no real data — skipping");
      return [];
    }
