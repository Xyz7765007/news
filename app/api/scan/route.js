...
async function scanNews(company, taskDefs) {
  ...
  if (items.length === 0) {
    console.log(`  [NEWS] No RSS, using AI fallback`);
    return generateAI(company, taskDefs, "news");
  }

  const enriched = await Promise.all(items.slice(0, 10).map(async n => ({ ...n, signalType: "news", articleContent: await fetchArticle(n.url) })));
...
async function scanJobs(company, taskDefs) {
  ...
  try {
    ...
    console.log(`  [JOBS] ${signals.length} valid signals after filtering`);
    return classify(signals, taskDefs, company.name, "jobs");
  } catch (e) {
...
async function generateAI(company, taskDefs, mode) {
  ...
  const prompt = mode === "jobs"
    ? `Generate 3-5 realistic JOB POSTING signals for a company. Each must match a task. Format headline as "Job Title — Company Name". Include realistic job description. Return ONLY JSON array: [{"headline":"CMO — Sprinto","description":"We are looking for...","source":"LinkedIn","url":"https://linkedin.com/jobs/view/...","date":"ISO","matchedTaskIds":["j1"],"confidence":0.85,"signalType":"job_post"}]`
    : `Generate 3-5 realistic NEWS headlines for a company. Each must match a task. Return ONLY JSON array: [{"headline":"...","description":"...","source":"Reuters","url":"https://reuters.com/...","date":"ISO","matchedTaskIds":["n1"],"confidence":0.85,"signalType":"news"}]`;
