...
function scanCompanyLocal(company, taskDefs, mode) {
  ...
  const count = Math.max(2, Math.min(2 + Math.floor(Math.random() * 3), taskDefs.length));
  const shuffled = [...taskDefs].sort(() => Math.random() - 0.5);
  const isJobs = mode === "jobs";
  return shuffled.slice(0, count).map((t) => {
    const headline = isJobs
      ? `${t.jobTitleKeywords?.[0] || t.name} — ${company.name}`
      : `${company.name}: ${t.name.toLowerCase()}`;
    const source = isJobs
      ? ["LinkedIn", "Indeed", "Glassdoor"][Math.floor(Math.random() * 3)]
      : ["Reuters", "Bloomberg", "TechCrunch", "Financial Times"][Math.floor(Math.random() * 4)];
    const domain = isJobs ? "linkedin.com/jobs" : "reuters.com/business";
  ...
  const applyAiSuggestion = () => {
    if (aiSuggestion && !aiSuggestion._clarify) {
      setForm(f => ({
        ...
        signalSource: aiSuggestion.signalSource || f.signalSource,
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{ maxWidth: 680 }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="input-group">
            <label className="input-label">Signal Source</label>
            <div style={{ display: "flex", gap: 6 }}>
              <button className={`btn btn-xs ${form.signalSource === "news" ? "btn-primary" : "btn-secondary"}`}
                style={{ background: form.signalSource === "news" ? "var(--green)" : undefined }}
                onClick={() => setForm(f => ({ ...f, signalSource: "news" }))}>
                📰 News Only
              </button>
              <button className={`btn btn-xs ${form.signalSource === "job_post" ? "btn-primary" : "btn-secondary"}`}
                style={{ background: form.signalSource === "job_post" ? "var(--blue)" : undefined }}
                onClick={() => setForm(f => ({ ...f, signalSource: "job_post" }))}>
                📋 Job Posts Only
              </button>
              <button className={`btn btn-xs ${form.signalSource === "both" ? "btn-primary" : "btn-secondary"}`}
                style={{ background: form.signalSource === "both" ? "var(--accent)" : undefined }}
                onClick={() => setForm(f => ({ ...f, signalSource: "both" }))}>
                📰📋 Both
              </button>
            </div>
          </div>

          {/* Job Title Keywords — shown for job_post and both */}
          {(form.signalSource === "job_post" || form.signalSource === "both") && (
            <div className="input-group">
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 6 }}>
                ...
                <span style={{ fontSize: 10, color: "var(--text-tertiary)" }}>AI will suggest the best LinkedIn job title search terms for this task</span>
              </div>
            </div>
          )}
        ...
        <div className="modal-footer">
          ...
          <button className="btn btn-primary" disabled={!form.name.trim()} onClick={() => {
            const prefix = form.signalSource === "job_post" ? "j_" : "n_";
            onSave({ ...form, id: form.id || `${prefix}${uid()}` });
          }}>
...
function AITaskBuilderChat({ onAddTask }) {
  ...
  const handleAcceptSuggestion = (suggestion) => {
    const prefix = suggestion.signalSource === "job_post" ? "j_" : suggestion.signalSource === "both" ? "b_" : "n_";
    const task = { ...suggestion, id: `${prefix}${uid()}` };
    onAddTask(task);
  ...
  const startScan = useCallback(async () => {
    ...
    if (scanRef.current && jobTasks.length > 0) {
      ...
      for (let i = 0; i < companies.length; i++) {
        ...
        const company = companies[i];
        setCurrentScanCompany(`${company.name} — Job Posts`);
        setScanProgress(((jobOffset + i) / totalSteps) * 100);

        const jobSignals = await scanCompanyAPI(company, jobTasks, "jobs");
        if (!scanRef.current) break;
        processSignals(jobSignals, company, "jobs");
        await sleep(100);
      }
    }
