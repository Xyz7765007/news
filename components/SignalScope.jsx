...
export default function SignalScope() {
  ...
  const startScan = useCallback(async () => {
    ...
    const newsTasks = taskDefs.filter(t => t.signalSource === "news" || (!t.signalSource && !t.id.startsWith("j")));
    const jobTasks = taskDefs.filter(t => t.signalSource === "job_post" || t.id.startsWith("j"));
    const totalSteps = companies.length * 2; // 2 phases per company
    ...
    if (scanRef.current && jobTasks.length > 0) {
      ...
      for (let i = 0; i < companies.length; i++) {
        ...
        setScanProgress(((companies.length + i) / totalSteps) * 100);
